 ...
 * 代理返利列表
 * **/
exports.agentRebateList = function (req, res, next) {
    // let _res = res;
    // let _req = req;
    // // // let header = {
    ...
    //     "page_no": page_no,
    // };
    request.get(`${API.manage.rebate}`)
        // .query({msg: JSON.stringify(params)})
        // .set(header)
        .end(function (req, res) {
            if (res.status === 200) {
                service.save_user_log({'type': 'getList', 'msg': '查询余额返利列表'}, _req);
            }
...
//     let _res = res;
//     let _req = req;
//     let {user_name, nickname, password, role_id, role_name} = req.body;
//     let header = {
//         token: _req.cookies.token
//     };
//     let params = {
...
//     let _res = res;
//     let _req = req;
//     let {id, user_name, nickname, password, role_id, role_name} = _req.body;
//     let header = {
//         token: _req.cookies.token
//     };
//
...
//             //记录用户操作日志
//             if (res.status == 200) {
//                 service.save_user_log({'type': 'add', 'msg': '新增' + params.key}, _req);
//             }
//             _res.json(JSON.parse(res.text))
//         })
//
...
//             //记录用户操作日志
//             if (res.status == 200) {
//                 service.save_user_log({'type': 'del', 'msg': '删除' + id}, _req);
//             }
//             _res.json(JSON.parse(res.text))
//         })
//
