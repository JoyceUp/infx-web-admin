<template>
    <section>
        <el-table stripe :data="items" style="width: 100%"  :header-row-style="headerRowStyle">
            ...
            <el-table-column label="MT4账号"> <template slot-scope="scope"> <span>{{isNull(scope.row.account_no)}}</span></template>    </el-table-column>
            <el-table-column label="资金类型"> <template slot-scope="scope"> <span>{{ changeStatus(scope.row.channel_type, channelTypeOptions) }}</span></template>  </el-table-column>
            <el-table-column label="申请金额(USD)"><template slot-scope="scope"> <span>{{dividedBy100(scope.row.amount)}}</span></template> </el-table-column>
            <el-table-column label="订单状态"><template slot-scope="scope"> <span>{{ changeStatus(scope.row.status, statusOptions) }}</span></template>         </el-table-column>
...
<script>
    import {agentDepDrawList} from '../../../api/agent_api';
    ...
    export default {
        ...
        methods: {
            ...
            showItems() {
                ...
                agentDepDrawList(params).then((res) => {
